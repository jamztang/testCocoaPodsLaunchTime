...
/// See https://pspdfkit.com/guides/ios/current/customizing-the-interface/document-presentation-options/ for further documentation.
...
typedef NS_CLOSED_ENUM(NSUInteger, PSPDFTextSelectionMode) {
    ...
    ///
    /// Additionally, on iOS 12 this mode uses two loupes and results
    /// either in a selection of one word or one image.
    ...
    ///
    /// Additionally, on iOS 12, this mode doesn't use the round loupe
    /// and can result in a selection containing multiple words.
    ...
    /// trackpad or mouse (on iOS 13.4 or later or on Mac Catalyst) will use simple
    /// mode.
...
typedef NS_CLOSED_ENUM(NSUInteger, PSPDFScrollInsetAdjustment) {
    ...
    /// Adjust scrooll view insets whenever user interface view elements are visible.
    PSPDFScrollInsetAdjustmentAllElements
} PSPDF_ENUM_SWIFT(ScrollInsetAdjustment);
...
typedef NS_CLOSED_ENUM(NSUInteger, PSPDFSignatureSavingStrategy) {
    /// Signature should be always saved.
    PSPDFSignatureSavingStrategyAlwaysSave,
    /// Signature should be never saved. Always showing the new signature creation UI.
    PSPDFSignatureSavingStrategyNeverSave,
    /// Signature should only be saved if selected in the UI.
...
@class PSPDFAnnotationGroup, PSPDFGalleryConfiguration, PSPDFDocumentEditorConfiguration;
...
///
/// @note This only has an effect if the page transition is either `PageTransition.scrollPerSpread`
/// or `PageTransition.scrollContinuous`.
///
/// @see `-[PSPDFStackViewLayout additionalContentInsets]`
...
/// Defaults to a dynamic color on iOS 13 and above which matches the current `UIUserInterfaceStyle`.
/// Falls back to dark gray color on versions prior iOS 13.
...
#pragma mark Scroll View Configuration
...
#pragma mark Page Border and Rendering
...
/// Defaults to `.search|.define|.actionSpeak|.actionShare|.actionCopy|.actionAnnotationCreation`.
...
#pragma mark User Interface Settings

/// Manages the show/hide mode of the user interface view. Defaults to `.automaticNoFirstLastPage`.
///
/// @note The user interface view consists of the thumbnail view at the bottom and the page/document label.
/// The visibility of the navigation bar of the parent navigation controller can be linked to the user interface via enabling `shouldHideNavigationBarWithUserInterface`.
///
/// @warning The user interface view will not change when changing this mode after controller is visible. Use `setUserInterfaceVisible:animated:` instead.
/// Does not affect manual calls to `setUserInterfaceVisible`.
...
/// Controls whether a `PDFViewController` should show a page grabber on the edge
...
#pragma mark Thumbnail Settings
...
/// The thumbnail size for `ThumbnailViewController`. Defaults to `CGSizeZero`.
...
#pragma mark Annotation Settings
...
/// If set to `true`, a long-tap that ends on a page area that is not a text/image will show a new menu to create annotations. Defaults to `true`.
/// If set to `false`, there's no menu displayed and the loupe is simply hidden.
...
/// Contains a list of `AnnotationToolConfiguration.ToolGroup` and `AnnotationToolConfiguration.ToolItem` items.
///
...
/// If `true`, the annotation menu will be displayed after an annotation has been created. Defaults to `false`.
...
/// See https:pspdfkit.com/guides/ios/current/annotations/annotation-and-bookmark-saving-triggers/ for details.
///
...
/// See https:pspdfkit.com/guides/ios/current/annotations/annotation-saving-mechanism/ for details.
///
...
/// Defines if a signature should be saved after creation. Defaults to `.saveIfSelected`.
/// Shows an action sheet when done creating a signature in `PDFSignatureViewController`,
/// when set to `.saveIfSelected`.
...
/// The default signature store implementation. Defaults to `KeychainSignatureStore`.
...
PSPDF_EXPORT UIActivityType const PSPDFActivityTypeOpenIn;
...
/// If enabled, will request that all thumbnails are pre-cached in `viewDidAppear:`. Defaults to `false`.
...
#pragma mark Gallery Configuration
...
/// @see PSPDFConfiguration.pageGrabberEnabled.
...
/// @see PSPDFConfiguration.showAnnotationMenuAfterCreation.
