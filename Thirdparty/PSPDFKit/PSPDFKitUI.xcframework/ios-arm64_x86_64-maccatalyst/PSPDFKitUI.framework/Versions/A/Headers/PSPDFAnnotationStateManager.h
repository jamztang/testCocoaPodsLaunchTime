...
/// Possible modes for `PSPDFAnnotationStateManager`’s `stylusMode` property.
...
/// `PSPDFAnnotationStateManager` holds the current annotation state and configures the associated `PDFViewController` to accept input related to the currently selected annotation state. The class also provides several convenience methods and user interface components required for annotation creation and configuration.
///
/// Interested parties can use KVO to observe the manager's properties.
///
/// You should never use more than one `PSPDFAnnotationStateManager` for any given `PDFViewController`. It's recommended to use `-[PSPDFViewController annotationStateManager]` instead of creating your own one in order to make sure this requirement is always met.
///
/// `PSPDFAnnotationStateManager` is internally used by `AnnotationToolbar` and can be re-used for any custom annotation related user interfaces.
///
...
///
/// This property will be nil on iOS 12.1 betas and earlier versions of iOS.
/// It will never be nil on versions from the 12.1 final release onwards.
@property (nonatomic, readonly, nullable) UIPencilInteraction *pencilInteraction API_AVAILABLE(ios(12.1));
