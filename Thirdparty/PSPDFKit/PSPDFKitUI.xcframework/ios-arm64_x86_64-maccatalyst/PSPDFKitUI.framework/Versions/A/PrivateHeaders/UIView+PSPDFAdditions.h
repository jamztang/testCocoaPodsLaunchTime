...
/// `standardAppearance` on iOS 13, or `barTintColor` on iOS 12.
///
...
///
/// @param bar A `UINavigationBar`, `UIToolbar` or any other view that responds to
/// `standardAppearance` on iOS 13, or `barTintColor` or `barStyle` on iOS 12.
///
...
@property (nonatomic) BOOL pspdf_focusRingEnabled;
...
- (nullable __kindof UIViewController *)pspdf_closestViewControllerOfClass:(Class)viewControllerClass;
...
- (nullable __kindof UIView *)pspdf_closestSuperviewOfClass:(Class)viewClass;

/// Queries subview hierarchy for a specific class. Faster than the prefix or suffix searches.
- (nullable __kindof UIView *)pspdf_closestSubviewOfClass:(nullable Class)viewClass NS_SWIFT_NAME(closestSubview(of:));
...
/// Performs the the block passed by using the view's trait collection as the current trait collection.
/// On iOS 12 and when the view trait collection is already set as the
/// current trait collection, the block operation directly.
...
/// On iOS 12, the block will not be executed at all.
