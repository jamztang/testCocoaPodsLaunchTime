...
/// The max number of pages that an user can export as images at once from a document.
#define PSPDFDocumentSharingAsImagePageLimit 20
...
/// The visible pages the user can pick from when sharing documents.
@property (nonatomic, readonly) PSPDFPageIndex firstVisiblePageIndex;
...
/// Sets the documents on the context, and notifies the delegates about it if `notifyDelefgates` is `true`.
///
/// @param documents           The new documents to set on the context.
/// @param notifyDelegates     Whether the delegaes should be notified about the documents update.
