...
/// The toolbar is positioned in the top (navigation)bar or below it. Deprecated in favor of `PSPDFFlexibleToolbarPositionVertical`.
PSPDF_EXPORT const PSPDFFlexibleToolbarPosition PSPDFFlexibleToolbarPositionsVertical PSPDF_DEPRECATED(9.3, 4.3, "Please use PSPDFFlexibleToolbarPositionVertical/.vertical instead.");
/// All valid toolbar positions. Excluding `PSPDFFlexibleToolbarPositionNone`. Deprecated in favor of `PSPDFFlexibleToolbarPositionAll`.
PSPDF_EXPORT const PSPDFFlexibleToolbarPosition PSPDFFlexibleToolbarPositionsAll PSPDF_DEPRECATED(9.3, 4.3, "Please use PSPDFFlexibleToolbarPositionAll/.all instead.");
...
///
/// When `PSPDFFlexibleToolbarPositionNone` is used (the default value) the the toolbar can select
...
/// Defaults to the`standardAppearance.backgroundColor` on iOS 13 onwards and `barTintColor` on earlier versionss if available, otherwise an attempt is made to select an appropriate color
...
/// Defaults to `PSPDFFlexibleToolbarPositionInTopBar`.
...
/// Includes `standardAppearance` and `compactAppearance` for iOS 13 and above only.
