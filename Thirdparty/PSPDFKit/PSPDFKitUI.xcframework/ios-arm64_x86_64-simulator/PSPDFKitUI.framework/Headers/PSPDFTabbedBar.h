...
/// A visual style of a tabbed bar.
/// This is not used on iOS 13 and above. Instead bar styles are based on `UIUserInterfaceStyle`.
typedef NS_CLOSED_ENUM(NSUInteger, PSPDFTabbedBarStyle) {
    /// Light style.
    PSPDFTabbedBarStyleLight,
    /// Dark style.
    PSPDFTabbedBarStyleDark,
} PSPDF_ENUM_SWIFT(PDFTabbedBarStyle);
...
PSPDF_CLASS_SWIFT(PDFTabbedBar)
...
/// This is ignored on iOS 13 and above. Instead the bar is styled based on the `traitCollection`’s `userInterfaceStyle`.
