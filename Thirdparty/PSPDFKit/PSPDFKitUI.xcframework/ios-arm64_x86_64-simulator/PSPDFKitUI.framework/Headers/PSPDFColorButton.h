...
@interface PSPDFColorButton (Deprecated)

/// If true, the shape will be an ellipse. If false, a rounded rectangle.
@property (nonatomic) BOOL displayAsEllipse PSPDF_DEPRECATED_IOS(9.3, "Please use `shape` instead.");

/// Border width. Defaults to 2.
@property (nonatomic) CGFloat borderWidth PSPDF_DEPRECATED_IOS(9.3, "Please use `outerBorderWidth` instead.");

/// Indicator size. Defaults to the bounds size.
@property (nonatomic) CGSize indicatorSize PSPDF_DEPRECATED_IOS(9.3, "Please use `contentInset` instead.");
