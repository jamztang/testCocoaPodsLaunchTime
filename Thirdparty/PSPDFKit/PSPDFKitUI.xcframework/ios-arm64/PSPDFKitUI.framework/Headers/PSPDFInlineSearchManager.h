...
#import <PSPDFKitUI/PSPDFSearchViewController.h> //HACK: imported because of 'PSPDFSearchStatus'
...
PSPDF_PROTOCOL_SWIFT(InlineSearchManagerDelegate)
...
/// `InlineSearchManager` manages the presentation of a search bar that may be used to find text in a `Document`.
/// The search bar sides down from the top, typically covering the navigation bar.
PSPDF_CLASS_SWIFT(InlineSearchManager)
