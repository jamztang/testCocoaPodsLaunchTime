...
/// Default container for annotations. It's crucial that you use this class as your base class if you implement a custom annotation provider, because
/// this class offers efficient undo/redo which otherwise is almost impossible to replicate unless you understand the PSPDFKit internals extremely well.
///
/// This class conforms to the deprecated protocol `PSPDFAnnotationProviderRefreshing`. In a future release, this protocol will be removed.
/// The reason is that this class handles refreshing internally. This class will continue to conform to `PSPDFAnnotationProvider`.
PSPDF_CLASS_SWIFT(PDFContainerAnnotationProvider)
@interface PSPDFContainerAnnotationProvider : NSObject <PSPDF_DEPRECATED_NOWARN(PSPDFAnnotationProviderRefreshing), PSPDFUndoSupport>
...
///
/// @warning This method **must be called within a write block!**
...
/// Deprecated methods from `PSPDFAnnotationProviderRefreshing`.
...
- (void)prepareForRefresh NS_REQUIRES_SUPER PSPDF_DEPRECATED(9.1, 4.1, "PSPDFKit handles refreshing internally now.");

/// This method is from `PSPDFAnnotationProviderRefreshing`. You must call super if you override this but ideally do not override this and instead let PSPDFKit handle refreshing internally.
- (void)refreshAnnotationsForPagesAtIndexes:(NSIndexSet *)pageIndexes NS_REQUIRES_SUPER PSPDF_DEPRECATED(9.1, 4.1, "PSPDFKit handles refreshing internally now.");
