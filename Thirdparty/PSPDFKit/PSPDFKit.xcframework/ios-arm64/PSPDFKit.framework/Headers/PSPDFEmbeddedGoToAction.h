...
typedef NS_CLOSED_ENUM(NSUInteger, PSPDFEmbeddedGoToActionTarget) {
    /// Not yet supported
    PSPDFEmbeddedGoToActionTargetParentOfCurrentDocument,
    PSPDFEmbeddedGoToActionTargetChildOfCurrentDocument,
} PSPDF_ENUM_SWIFT(EmbeddedGoToAction.Target);
...
PSPDF_CLASS_SWIFT(EmbeddedGoToAction)
@interface PSPDFEmbeddedGoToAction : PSPDFGoToAction

/// Initialize the embedded GoTo action with a `remotePath` and settings.
- (instancetype)initWithRelativePath:(NSString *)remotePath targetRelationship:(PSPDFEmbeddedGoToActionTarget)targetRelationship openInNewWindow:(BOOL)openInNewWindow pageIndex:(PSPDFPageIndex)pageIndex;
