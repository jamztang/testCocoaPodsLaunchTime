...
/// Renamed. Use `PSPDFSignerErrorCode` instead.
typedef PSPDFSignerErrorCode PSPDFSignerError PSPDF_DEPRECATED(9.4, 4.4, "Renamed. Use `PSPDFSignerErrorCode` instead.");
...
///
/// Set `signatureType` to modify the subFilter.
@property (nonatomic, readonly) NSString *subFilter PSPDF_DEPRECATED(10.1, 5.1, "Use `signatureType` instead.");

/// The PDF Digital Signature type. This translated directly to signature "SubFilter" type in the PDF
/// .
/// Returns `PSPDFSignatureTypeCMS` by default, as it's the only value fully supported for now.
/// Other values are `PSPDFSignatureTypeSHA1` (deprecated), and `PSPDFSignatureTypePades` for
...
- (void)requestSigningCertificate:(id)sourceController completionBlock:(nullable void (^)(PSPDFX509 *_Nullable x509, NSError *_Nullable error))completionBlock PSPDF_UNAVAILABLE_MAC;
