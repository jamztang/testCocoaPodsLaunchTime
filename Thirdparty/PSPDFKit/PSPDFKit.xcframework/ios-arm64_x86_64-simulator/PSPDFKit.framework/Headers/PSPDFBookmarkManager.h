...
typedef NS_CLOSED_ENUM(NSUInteger, PSPDFBookmarkManagerSortOrder) {
...
} PSPDF_ENUM_SWIFT(BookmarkManager.SortOrder);
...
/// # Subclassing
///
/// You should not subclass `BookmarkManager`. Instead attach a custom bookmark
/// provider to achieve your desired behavior.
///
/// # Thread Safety
///
/// `BookmarkManager` is thread safe and can be accessed from any thread. To ensure
...
/// block calls.
PSPDF_CLASS_SUBCLASSING_RESTRICTED_SWIFT(BookmarkManager)
...
#pragma mark Accessing Bookmarks
...
#pragma mark Modifying Bookmarks
...
/// adjusts the index of the bookmarks in between.
///
...
#pragma mark Combining Multiple Operations
...
#pragma mark Customizing Bookmark Providers
