...
///
/// This type applies to both signatures added using a certificate (digital signatures) and
/// those added without a certificate, which end up as drawings (`PSPDFAnnotationStringInk`).
/// You can check if an ink annotation is a signature using the `isSignature` property,
...
/// Use `Annotation.ToolVariantID(tool:variant:)` to generate an identifer in Swift.
...
typedef NS_OPTIONS(NSUInteger, PSPDFAnnotationFlags) {
    ...
    /// If set, don't allow the annotation to interact with the user. Ignored for Widget annotations.
    /// For widget annotations its function is subsumed by the `ReadOnly` flag of the associated form field.
...
@protocol PSPDFAppearanceStreamGenerating;
...
/// The key is of type `PSPDFAnnotation.TriggerEvent`.
...
/// @note `appearanceStreamGenerator` is not supported on `PSPDFSignatureFormElements`. Please use `PSPDFSignatureAppearance`.
/// @note If you intend to use `appearanceStreamGenerator` on a annotation subclass that has `drawInContext:options:` overridden, you have to
/// call `maybeRenderCustomAppearanceStreamWithContext:options:` and return if it returns true.
...
/// Draws the annotation in a graphics context. Coordinates here are in the normalized PDF coordinate space.
...
} PSPDF_ENUM_SWIFT(VerticalAlignment);
...
///
/// @param instantJSON An `NSData` object containing the Instant Annotation JSON. For example, `NSData *data = [jsonString dataUsingEncoding:NSUTF8StringEncoding];`
...
/// @return An `NSData` object containing the Instant JSON representation of the receiver. Can be converted into JSON string like so: `NSString *jsonString = [[NSString alloc] initWithData:result encoding:NSUTF8StringEncoding];`
...
/// @param dataProvider The data provider with the binary Instant JSON data.
/// @return YES if successful, otherwise NO.
- (BOOL)attachBinaryInstantJSONAttachmentFromDataProvider:(id<PSPDFDataProviding>)dataProvider mimeType:(nullable NSString *)mimeType error:(NSError **)error PSPDF_DEPRECATED(9.1, 4.1, "Use attachBinaryInstantJSONAttachmentFromDataProvider:error");
