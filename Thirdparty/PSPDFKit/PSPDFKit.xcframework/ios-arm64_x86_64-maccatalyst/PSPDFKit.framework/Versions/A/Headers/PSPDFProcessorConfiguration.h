...
/// Merges a page from a source document with a page of the configuration's document.
///
...
///
/// @note This method requires the Comparison component to be enabled for your license.
- (void)mergePageFromDocument:(PSPDFDocument *)sourceDocument password:(nullable NSString *)password sourcePageIndex:(PSPDFPageIndex)sourcePageIndex destinationPageIndex:(PSPDFPageIndex)destinationPageIndex transform:(CGAffineTransform)transform blendMode:(CGBlendMode)blendMode PSPDF_DEPRECATED(9.4, 4.4, "Please use  `mergeAutoRotatedPageFromDocument`");
