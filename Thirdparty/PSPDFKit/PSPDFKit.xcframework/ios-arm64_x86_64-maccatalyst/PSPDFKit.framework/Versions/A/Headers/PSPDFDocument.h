...
@class PSPDFAnnotationManager, PSPDFBookmarkManager, PSPDFDocumentProvider, PSPDFEmbeddedFile, PSPDFFormParser, PSPDFOutlineParser, PSPDFPageInfo, PSPDFRenderReceipt, PSPDFTextParser, PSPDFTextSearch, PSPDFViewController, PSPDFFile, PSPDFProgress, PSPDFDocumentCheckpointer, PSPDFOutlineElement, PSPDFDocumentFeatures;
...
/// `PSPDFCoordinatedFileDataProvider` by default. You can also opt to use `NSData` or custom implementations of
/// `PSPDFDataProviding` as sources during initialization.
///
...
///
/// This convenience initializer creates either a `PSPDFCoordinatedFileDataProvider` or a `PSPDFFileDataProvider`
...
///
/// @param URL A local file URL of a PDF document.
/// @return A new document object.
- (instancetype)initWithURL:(NSURL *)URL;

/// Initialize `Document` with one or multiple `dataProviders` (id<PSPDFDataProviding>).
///
...
/// Initialize `Document` with one or multiple `dataProviders` (id<PSPDFDataProviding>), optionally enabling restoring from a checkpoint.
///
/// To load from a local file, use `PSPDFCoordinatedFileDataProvider`.
/// To load from `NSData`, use `PSPDFDataContainerProvider`.
///
...
///
/// @param dataProviders An array containing objects conforming to `PSPDFDataProviding` to be added to the new document.
/// @return A new document object.
...
///
/// @note Will *always* be appended by UID. Don't manually append UID.
...
///
/// This property is initialized to the value stored in `NSUserDefaults.standardUserDefaults`
/// under the `PDFDocumentDefaultAnnotationUsernameKey`. If unset, a guess is made based
/// on the device name.
///
/// Written as the "T" (title/user) property of newly created annotations.
///
/// This value is also used by `PSPDFNoteAnnotationViewController` to determine
...
///  @param block    Returns the options to apply to the given type.
...
/// @return The generated JSON as an `NSData` object, or nil if there are no changes in the document currently, or on error occurred.
...
///
/// @note Set this before `undoController` is first accessed.
...
@property (atomic, readonly, nullable) PSPDFUndoController *undoController;
