...
/// This method should replace your current data with the one written into `replacementDataSink`.
///
/// When called by PSPDFKit, `replacementDataSink` is the object instantiated in `createDataSink(options:)`.
/// Depending on `replacementDataSink.options`, you either have to append or replace the data.
- (BOOL)replaceWithDataSink:(id<PSPDFDataSink>)replacementDataSink error:(NSError * __autoreleasing *)error PSPDF_DEPRECATED(9.3, 4.3, "Please rename your implementation to `replaceContents(with:)`/`-replaceContentsWithDataSink:error:`");
