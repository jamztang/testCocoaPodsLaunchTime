...
// We use this in some places to silence deprecation warnings.
#define PSPDF_HACK_TEMPORARILY_SILENCE_DEPRECATED_WARNINGS_BEGIN _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wdeprecated-declarations\"")
#define PSPDF_HACK_TEMPORARILY_SILENCE_DEPRECATED_WARNINGS_END _Pragma("clang diagnostic pop")
...
/// Ensures object is not nil. Asserts if this is not the case.
/// Useful as a forced nullable -> nonnull cast.
#define PSPDFNN(thing)                                              \
    ^{                                                              \
        __auto_type _Null_unspecified thang = thing;                \
        NSCAssert(thang != nil, @"'" #thing "' must exist"); \
        return thang;                                               \
    }()

/// Asserts that an id pointer is not nil. PSPDFNN uses __auto_type which can’t be used with id.
