...
+ (nullable NSString *)pspdf_anyUTITypeFromFilename:(NSString *)fileName NS_SWIFT_NAME(anyUTIType(from:));

/// Returns all possible types for the extension of the given filename.
+ (NSArray<NSString *> *)pspdf_allUTITypesFromFilename:(NSString *)fileName NS_SWIFT_NAME(allUTITypes(from:));

/// Ensures that the filename conforms to the type provided.
///
/// @note The receiver needs to be a filename with a valid extension.
- (BOOL)pspdf_conformsToUTIType:(NSString *)utiType NS_SWIFT_NAME(conforms(toUTIType:));
