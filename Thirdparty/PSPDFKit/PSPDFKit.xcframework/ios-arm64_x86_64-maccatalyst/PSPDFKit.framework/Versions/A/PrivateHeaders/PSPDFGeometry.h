...
PSPDF_EXTERN CGRect PSPDFCalculateMinimumBoundingRect(CGPoint *points, NSUInteger count);

/// Calcualtes the minimum smallest rect that encompasses all the input points.
///
/// @param boxedPoints An array of `NSValue`s containing CGPoints.
/// @return The smallest bounding rect for the input.
PSPDF_EXTERN CGRect PSPDFCalculateMinimumBoundingRectFromBoxedPoints(NSArray<NSValue *> *boxedPoints);

/// Returns the smallest rectange that encloses all the passed in rectangles.
...
/// Returns the shortest distance between the edge of a rectangle and a point outside that rectange, or 0 if the point is inside the rectangle.
...
PSPDF_SDK_EXPORT bool PSPDFSizeIsEmpty(CGSize size);
