...
// Safe way to count items in a C array.
// https://stackoverflow.com/questions/4415524/common-array-length-macro-for-c
#define PSPDFArrayCount(x) ((sizeof(x) / sizeof(0 [x])) / ((size_t)(!(sizeof(x) % sizeof(0 [x])))))
...
- (nullable id)pspdf_foldLeftWithValue:(nullable id)startingValue usingBlock:(NS_NOESCAPE id _Nullable (^)(id _Nullable left, ObjectType right))block;
...
/// @param block A block that describes how to combine elements of the receiver.
/// If the receiver is empty, this block will never be invoked.
- (nullable id)pspdf_foldRightWithValue:(nullable id)startingValue usingBlock:(NS_NOESCAPE id _Nullable (^)(ObjectType left, id _Nullable right))block;
...
@property (nonatomic, readonly) NSPointerArray *pspdf_weakArray;

/// Removes any duplicates from the array.
@property (nonatomic, readonly) NSArray<ObjectType> *pspdf_uniqueArray;
