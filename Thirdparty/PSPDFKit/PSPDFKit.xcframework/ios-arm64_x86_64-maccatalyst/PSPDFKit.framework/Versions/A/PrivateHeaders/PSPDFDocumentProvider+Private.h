...
@class PDFCDocumentProvider, PDFCPage, PSPDFAnnotation, PSPDFCoreFormProvider, PSPDFDispatchQueue, PSPDFPageInfo, PSPDFResourceManager, PSPDFUndoController;
...
/// Initialize with an object conforming to `PSPDFDataProviding`.
- (instancetype)initWithDataProvider:(id<PSPDFDataProviding>)dataProvider index:(NSUInteger)index document:(PSPDFDocument *)document coreDocumentProvider:(nullable PDFCDocumentProvider *)coreDocumentProvider contentSignature:(nullable NSData *)contentSignature isLocked:(BOOL)isLocked;
...
/// Changes the default behavior to ignore missing files.
@property (nonatomic) BOOL checkIfFileExists;
...
///
/// Invalidation requires orderly deregistering from the undo controller, and may have to happen at times when the weak
/// back-reference to the document is already gone!
///
