...
PSPDF_APP_EXPORT void PSPDFExecuteApplicationBackgroundTask(NSString *name, void(NS_NOESCAPE ^ block)(void(NS_NOESCAPE ^ completionBlock)(void)));

/// Always returns `false` on Mac and a result based on `UIApplicationState` on iOS.
///
/// Simplifies cross platform code.
PSPDF_APP_EXPORT BOOL PSPDFApplicationInBackground(void);
...
PSPDF_APP_EXPORT BOOL PSPDFApplicationIsTerminating(void);

/// Returns YES if we're inside an XCTestCase.
PSPDF_APP_EXPORT BOOL PSPDFApplicationIsRunningTests(void);

/// Returns the slowest qualiy of service. Background for apps, Utility for tests.
PSPDF_TEST_EXPORT NSQualityOfService PSPDFSlowestQualityOfService(void);
