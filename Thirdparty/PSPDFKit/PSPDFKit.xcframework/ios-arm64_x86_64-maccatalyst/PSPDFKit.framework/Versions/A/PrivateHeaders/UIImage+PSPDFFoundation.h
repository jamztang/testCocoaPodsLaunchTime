...
- (UIImage *)pspdf_resizedImageWithContentMode:(UIViewContentMode)contentMode bounds:(CGSize)bounds honorScaleFactor:(BOOL)honorScaleFactor interpolationQuality:(CGInterpolationQuality)quality NS_SWIFT_NAME(resizedImage(contentMode:bounds:honorScaleFactor:interpolationQuality:));

/*
 `Uses the `imageWithTintColor` API available since iOS 13 underneath
 which when given a dynamic color returns an image that
 redraws itself on trait collection changes resolving the dynamic color.

 @note On versions prior iOS 13, it returns the same image.
 */
- (UIImage *)pspdf_imageWithTintColor:(UIColor *)color NS_SWIFT_NAME(tinted(dynamicColor:));
